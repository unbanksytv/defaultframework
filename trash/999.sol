...
// File contracts/GVOGenesisERC1155.sol
...
contract GVOGenesisERC1155 is ERC1155, Ownable {
    ...
    string public symbol = "GVOGENESIS";
    ...
    constructor (address _treasury, address _sGVO, string memory _uri) ERC1155(_uri) {
        treasury = _treasury;
        sGVO = _sGVO;
    }
    ...
    function setsGVO(address _sGVO) external onlyOwner {
        sGVO = _sGVO;
    }
    ...
    function toggleSaleIsActive() external onlyOwner {
        if (!saleIsActive) {
            require(
                IERC20Contract( sGVO ).balanceOf(address(this)) >= 33 * (totalRemaining1 + totalRemaining2 + totalRemaining3) * 10 ** 9, 
                "NOT_ENOUGH_SGVO"
    ...
    function mint(uint256 _numHoodies) external payable {
        ...
        IERC20Contract( sGVO ).transfer(msg.sender, _numHoodies * 33 * 10 ** 9);
    ...
    function withdrawAllsGVO() public onlyOwner {
        require(!saleIsActive, "NO_WITHDRAW_DURING_SALE");
        uint256 sGVOBalance = IERC20Contract( sGVO ).balanceOf(address(this));
        require(sGVOBalance > 0, "NO_SGVO_BALANCE");
        IERC20Contract( sGVO ).transfer(treasury, sGVOBalance);
    }
}
